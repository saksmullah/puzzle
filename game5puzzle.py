...
def createboard(number_of_rows, number_of_coloums, number_of_cells):
   ...
   for i in range(cfg.random):#this is the file made from the cfg file and random is 100 as our range
...
def show_start_interface(screen, width, height):
   screen.fill(cfg.background_colour)
   t_font = pygame.font.Font(cfg.fontpath, width // 4)
   c_font = pygame.font.Font(cfg.fontpath, width // 20)
   title = t_font.render('Puzzle', True, cfg.red_colour)
   content_1 = c_font.render("Press H, M or L to choose your puzzle", True, cfg.blue_colour)
   content_2 = c_font.render("H - 5x5, M - 4x4, L - 3x3", True, cfg.blue_colour)
...
def main():
   ...
   game_image_used = pygame.image.load(get_images_path(cfg.picture_root_dir))
   game_image_used = pygame.transform.scale(game_image_used, cfg.screenzize)
   game_image_used_rect = game_image_used.get_rect()

   screen = pygame.display.set_mode(cfg.screenzize)
